...
END $$;
GRANT EXECUTE ON FUNCTION public.riwayat_dengan_petugas(uuid) TO authenticated, anon;
...
INSERT INTO storage.buckets (id, name, public) VALUES ('pejabat-foto','pejabat-foto',true) ON CONFLICT (id) DO NOTHING;
...
CREATE POLICY "Foto pejabat publik baca" ON storage.objects FOR SELECT TO public USING (bucket_id='pejabat-foto');
...
CREATE POLICY "Super admin upload foto pejabat" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id='pejabat-foto' AND has_role(auth.uid(),'super_admin'));
...
CREATE POLICY "Super admin update foto pejabat" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id='pejabat-foto' AND has_role(auth.uid(),'super_admin'));
