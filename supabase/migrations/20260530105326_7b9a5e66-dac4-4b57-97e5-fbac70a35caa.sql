...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT DISTINCT permission_code FROM public.user_permissions
...
CREATE POLICY "form_fields super admin" ON public.form_fields FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'super_admin')) WITH CHECK (public.has_role(auth.uid(),'super_admin'));
CREATE POLICY "form_fields owner kelola" ON public.form_fields FOR ALL TO authenticated
  USING (EXISTS(SELECT 1 FROM public.forms f WHERE f.id = form_id AND (f.created_by = auth.uid() OR (public.has_role(auth.uid(),'admin_opd') AND f.opd_pemilik_id = public.get_user_opd(auth.uid())))))
  WITH CHECK (EXISTS(SELECT 1 FROM public.forms f WHERE f.id = form_id AND (f.created_by = auth.uid() OR (public.has_role(auth.uid(),'admin_opd') AND f.opd_pemilik_id = public.get_user_opd(auth.uid())))));
...
CREATE POLICY "form_targets super admin" ON public.form_targets FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'super_admin')) WITH CHECK (public.has_role(auth.uid(),'super_admin'));
CREATE POLICY "form_targets owner kelola" ON public.form_targets FOR ALL TO authenticated
  USING (EXISTS(SELECT 1 FROM public.forms f WHERE f.id = form_id AND (f.created_by = auth.uid() OR (public.has_role(auth.uid(),'admin_opd') AND f.opd_pemilik_id = public.get_user_opd(auth.uid())))))
  WITH CHECK (EXISTS(SELECT 1 FROM public.forms f WHERE f.id = form_id AND (f.created_by = auth.uid() OR (public.has_role(auth.uid(),'admin_opd') AND f.opd_pemilik_id = public.get_user_opd(auth.uid())))));
...
CREATE POLICY "form_assignments lihat sendiri/admin" ON public.form_assignments FOR SELECT TO authenticated
  USING (user_id = auth.uid() OR public.has_role(auth.uid(),'super_admin')
    OR EXISTS(SELECT 1 FROM public.forms f WHERE f.id = form_id AND public.has_role(auth.uid(),'admin_opd') AND f.opd_pemilik_id = public.get_user_opd(auth.uid())));
CREATE POLICY "form_assignments super admin" ON public.form_assignments FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'super_admin')) WITH CHECK (public.has_role(auth.uid(),'super_admin'));
CREATE POLICY "form_assignments owner kelola" ON public.form_assignments FOR ALL TO authenticated
...
CREATE POLICY "fsf lihat via submission" ON public.form_submission_files FOR SELECT TO authenticated
  USING (EXISTS(SELECT 1 FROM public.form_submissions s WHERE s.id = submission_id AND (
    s.user_id = auth.uid() OR public.has_role(auth.uid(),'super_admin')
    OR EXISTS(SELECT 1 FROM public.forms f WHERE f.id = s.form_id AND public.has_role(auth.uid(),'admin_opd') AND f.opd_pemilik_id = public.get_user_opd(auth.uid()))
  )));
